...
public class HorizontalBarChartRenderer extends BarChartRenderer {
    ...
    @Override
    public void drawHoriDescrib(Canvas c) {
        super.drawHoriDescrib(c);
        BarData barData = mChart.getBarData();
        for (int index = 0; index < barData.getDataSetCount(); index++) {

            IBarDataSet dataSet = barData.getDataSetByIndex(index);

            if (dataSet.isVisible()) {
                drawHoriDescript(c, dataSet, index);
            }
        }
    }

    private void drawHoriDescript(Canvas c, IBarDataSet dataSet, int index) {
        ...
        String[] valueDescrip = dataSet.getmHoriDescrip();
...
//                mRenderPaint.setColor(dataSet.getColor(j / 4));
            ...
            if (valueDescrip != null && valueDescrip.length == buffer.size() / 4) {
                ...
                c.drawText(valueDescrip[j / 4], buffer.buffer[j], buffer.buffer[j + 1] - radiu / 2, mRenderPaint);
            }
        }
    }
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        String[] valueDescrip = dataSet.getStackLabels();
        for (int j = 0; j < buffer.size(); j += 4) {
...
//            if(valueDescrip != null && valueDescrip.length == buffer.size()/4) {
//                mRenderPaint.setTextSize(20);
//                c.drawText(valueDescrip[j/4], buffer.buffer[j], buffer.buffer[j + 1] - 20, mRenderPaint);
//            }
        }
    }
